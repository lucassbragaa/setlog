...
export function HistoryScreen({ history, onDelete }: { history: WorkoutSession[]; onDelete: (id: string) => void }) {
  ...
  return (
    <ScrollView contentContainerStyle={commonStyles.screen}>
      <ScreenTitle eyebrow="REGISTRO COMPLETO" title="Histórico" subtitle={history.length + ' sessões salvas'} />
        ...
        return (
          <Pressable key={session.id} style={commonStyles.card} onPress={() => setSelected(open ? null : session.id)}>
            <View style={commonStyles.between}>
              <View><Text style={commonStyles.cardTitle}>{session.name}</Text><Text style={commonStyles.muted}>{new Date(session.startedAt).toLocaleDateString('pt-BR')} · {session.exercises.length} exercícios</Text></View>
